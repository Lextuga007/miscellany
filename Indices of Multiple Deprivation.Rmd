...
It may be appropriate to rank the IMD areas locally rather than use those ranked across England. For example, in Nottingham the differences between the LSOA areas is diminished when ranked against England as a whole, but when ranked within Nottingham and Nottinghamshire, the variation is much more pronounced.

To do this in SQL the data needs to be restricted to the appropriate area, for example when joining to the Postcodes and restricting and then a windows partition applied to the data ROW_NUMBER() OVER(ORDER BY IMDRank) to create a new ranking score and NTILE(10) OVER (ORDER BY IMDRank) to create new deciles.
